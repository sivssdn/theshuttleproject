<?php
 ...
 //program to register user for seats
 
 
 //variables to get the data from form
 $campus_time=$_GET['jahangirpuri'];
 $compare_time=htmlspecialchars($_GET['compare_t2']);
 $campus_date=htmlspecialchars($_GET['date2']);
 $insert_at=$campus_date[0].$campus_date[1];
 $day=htmlspecialchars($_GET['day2']);
 $table=$day."tojahangir";
 $do=htmlspecialchars($_GET['do']);
 //for people from parker
 $parker=htmlspecialchars($_GET['parker']);
 $status="CAMPUS";
 if($parker=="yes")
 $status="PARKER";
 ...
 try{
	...
	*/
	...
	$seats_present=$db->prepare("SELECT `{$campus_time}` from `addshuttle_towardsjahangir` WHERE `days` LIKE '{$add_shuttle_day}';");
	...
	if($seats_count[0]>0 && $allow)
	{
		...
		$data=$db->prepare("SELECT count(serial) FROM people2 WHERE email LIKE '{$_SESSION['id']}' AND date='{$date_db}' AND time LIKE '{$campus_time}';");
		$data->execute();
		$total=$data->fetch();
		
	    //allowing only one entry per used  ::[change]::100 to 1 =-=-=
		if($total[0]<1 || $_SESSION['id']=="Admin sahab")
		{
			$db->exec("UPDATE `addshuttle_towardsjahangir` SET `{$campus_time}`={$seats_count[0]}-1 WHERE `days`='{$add_shuttle_day}';");
			...
			$db->exec("INSERT INTO `shuttle`.`people2` (`serial`, `name`, `email`, `date`, `time`, `status`) VALUES (Null, '{$_SESSION['name']}', '{$_SESSION['id']}', '{$date_db}', '{$campus_time}', '{$status}');");
			$booking="CONFIRMED";
			$available=$seats_count[0]-1; //to output seats remaining
		}
		else
		{
			$booking="exists";
	...
	else if($allow)
	{
		...
		$date_d=$campus_date[6].$campus_date[7].$campus_date[8].$campus_date[9].'-'.$campus_date[3].$campus_date[4].'-'.$campus_date[0].$campus_date[1];
		$list_db->exec("INSERT INTO `shuttle`.`waitlist2` (`serial`, `name`, `email`, `date`, `time`, `status`) VALUES (NULL, '{$_SESSION['name']}', '{$_SESSION['id']}', '{$date_d}', '{$campus_time}', '{$status}');");
		
		
		//to check waitlist number
		$number=$list_db->prepare("SELECT count(serial) FROM waitlist2 WHERE `time` LIKE '{$campus_time}' AND `date`='{$date_d}';");
		$number->execute();
 ...
 try{
	...
	*/
	...
	$number=$list_db->prepare("SELECT count(serial) FROM waitlist2 WHERE `time` LIKE '{$campus_time}' AND `date`='{$date_date}';");
	$number->execute();
	$waitlist=$number->fetch();
