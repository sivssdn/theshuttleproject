...
//when form is submitted making an update request
	...
	if(isset($_POST['submit']))
	{
		...
		$db->exec("UPDATE `shuttle`.`addshuttle_towardscampus` SET `{$utime}` = '{$increaseto}' WHERE `addshuttle_towardscampus`.`days` LIKE '{$_SESSION['day']}';");
		
		for($i=0; $i<$uwait; $i++)
		{
		//transfer one entry from waitlist to people
			 $transfer=$waitlist->prepare("SELECT name,email,date,time FROM waitlist WHERE date='{$date_today}' AND time LIKE '{$utime}';");
			 $transfer->execute();
			 $first_row=$transfer->fetch();
			 
			 //update confirmed bookings
			 $people->exec("INSERT INTO `people` (`serial`, `name`, `email`, `date`, `time`, `status`) VALUES (NULL, '{$first_row['name']}', '{$first_row['email']}', '{$first_row['date']}', '{$first_row['time']}', '{$first_row['status']}');");
			 
			 //delete entry from waitlist
			 $waitlist->exec("DELETE FROM waitlist WHERE `name` LIKE '{$first_row['name']}' AND `email` LIKE '{$first_row['email']}' AND `date`='{$first_row['date']}' AND `time` LIKE '{$first_row['time']}' LIMIT 1;"); 
		
		}
		//to reset column value to 12
		$col=$db->prepare("describe addshuttle_towardscampus;");
		$col->execute();
		...
		foreach($rescol as $valcol)
		{
			if($valcol != $utime)
			{
				...
				$seats_present=$db->prepare("SELECT `{$valcol}` from addshuttle_towardscampus WHERE `days` LIKE '{$_SESSION['day']}';");
				$seats_present->execute();
				$seats_count=$seats_present->fetch();
				if($seats_count[0]!=0)
				{
					$db->exec("UPDATE `shuttle`.`addshuttle_towardscampus` SET `{$valcol}` = '12' WHERE `addshuttle_towardscampus`.`days` LIKE '{$_SESSION['day']}';");
				}
	...
	<table id="large">
			<?php
				...
				$db->beginTransaction();
					...
					$mon=$db->prepare("describe addshuttle_towardscampus;");
					$mon->execute();
					...
					{		//::::::::::change time here 18:30
							...
							$seats_booked=$people->prepare("SELECT count(serial) FROM people WHERE `time` LIKE '{$res}' AND `date`='{$today_date}';");
							$seats_booked->execute();
							$seats_total=$seats_booked->fetch();
							
							//getting count of waitlistss
							$waitlist_booked=$waitlist->prepare("SELECT count(serial) FROM waitlist WHERE `time` LIKE '{$res}' AND `date`='{$today_date}';");
							...
							$seats_present=$db->prepare("SELECT `{$res}` from addshuttle_towardscampus WHERE `days` LIKE '{$_SESSION['day']}';");
							$seats_present->execute();
							$seats_count=$seats_present->fetch();
